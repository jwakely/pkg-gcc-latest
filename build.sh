...
usage()
{
  echo "Usage: $0 <snapshot> <spec|srpm|rpm|deb|all|upload|info>"
...
gen_spec()
{
  ...
  m4 -P -DVERSION=$BASE_VER -DPKGREL=5 -DSNAPINFO=${DATE}git${REV} -DSOURCE_URL=https://gcc.gnu.org/pub/gcc/snapshots/${basename#gcc-}/$tarfile -DBASENAME=$basename gcc-latest.spec.m4 > gcc-latest.spec
}
